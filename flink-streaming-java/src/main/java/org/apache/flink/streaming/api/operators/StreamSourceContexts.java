...
public class StreamSourceContexts {
	...
	/**
	 * Same as above but including a SchedulerMonitor object
	 */
	public static <OUT> SourceFunction.SourceContext<OUT> getSourceContext(
			TimeCharacteristic timeCharacteristic,
			ProcessingTimeService processingTimeService,
		...
		SourceFunction.SourceContext<OUT> ctx =
				getSourceContext(timeCharacteristic, processingTimeService,
						checkpointLock, streamStatusMaintainer, output, watermarkInterval, idleTimeout);
		((WatermarkContext) (ctx)).setSchedulerMonitor(schedulerMonitor);
		return ctx;
	}
	...
	private abstract static class WatermarkContext<T> implements SourceFunction.SourceContext<T> {
		...
		public void collect(T element) {
			synchronized (checkpointLock) {
				...
				if (schedulerMonitor != null && !schedulerMonitor.isScheduled()) {
