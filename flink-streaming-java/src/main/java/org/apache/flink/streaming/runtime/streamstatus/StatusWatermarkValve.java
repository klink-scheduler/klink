...
public class StatusWatermarkValve {
	...
	private Watermark lastOutputWatermark;
	...
	public StatusWatermarkValve(int numInputChannels, ValveOutputHandler outputHandler) {
		...
		for (int i = 0; i < numInputChannels; i++) {
			...
			channelStatuses[i].watermark = new Watermark(Long.MIN_VALUE);
		...
		this.lastOutputWatermark = new Watermark(Long.MIN_VALUE);
	...
	public void inputWatermark(Watermark watermark, int channelIndex) {
		...
		if (lastOutputStreamStatus.isActive() && channelStatuses[channelIndex].streamStatus.isActive()) {
			...
			if (watermarkMillis > channelStatuses[channelIndex].watermark.getTimestamp()) {
				...
				if (!channelStatuses[channelIndex].isWatermarkAligned && watermarkMillis >= lastOutputWatermark.getTimestamp()) {
	...
	public void inputStreamStatus(StreamStatus streamStatus, int channelIndex) {
		...
		if (streamStatus.isIdle() && channelStatuses[channelIndex].streamStatus.isActive()) {
			...
			if (!InputChannelStatus.hasActiveChannels(channelStatuses)) {
				...
				if (channelStatuses[channelIndex].watermark.getTimestamp() == lastOutputWatermark.getTimestamp()) {
			...
			} else if (channelStatuses[channelIndex].watermark.getTimestamp() == lastOutputWatermark.getTimestamp()) {
	...
	private void findAndOutputNewMinWatermarkAcrossAlignedChannels() {
		Watermark newMinWatermark = new Watermark(Long.MAX_VALUE);
		...
		for (InputChannelStatus channelStatus : channelStatuses) {
			if (channelStatus.isWatermarkAligned) {
				...
				if(channelStatus.watermark.getTimestamp() < newMinWatermark.getTimestamp()) {
					newMinWatermark = channelStatus.watermark;
				}
			}
		}
		...
		if (hasAlignedChannels && newMinWatermark.getTimestamp() > lastOutputWatermark.getTimestamp()) {
			lastOutputWatermark = newMinWatermark;
			outputHandler.handleWatermark(lastOutputWatermark);
		}
	}

	private void findAndOutputMaxWatermarkAcrossAllChannels() {
		Watermark maxWatermark = new Watermark(Long.MIN_VALUE);

		for (InputChannelStatus channelStatus : channelStatuses) {
			if (channelStatus.watermark.getTimestamp() > maxWatermark.getTimestamp()) {
				maxWatermark = channelStatus.watermark;
			}
		}

		if (maxWatermark.getTimestamp() > lastOutputWatermark.getTimestamp()) {
