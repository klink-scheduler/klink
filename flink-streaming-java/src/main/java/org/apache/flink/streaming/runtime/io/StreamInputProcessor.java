...
public class StreamInputProcessor<IN> {
	...
	private Counter numWatermarkIn;
	private Counter numLatencyMarkersIn;

	private boolean isFinished;

	private SchedulerCollector schedulerCollector;
	...
	public void setSchedulerCollector(SchedulerCollector schedulerCollector){
	...
	public boolean processInput() throws Exception {
		...
		if (numWatermarkIn == null) {
			try {
				numWatermarkIn = ((OperatorMetricGroup) streamOperator.getMetricGroup()).getIOMetricGroup().getNumWatermarksIn();
			} catch (Exception e) {
				LOG.warn("An exception occurred during the metrics setup.", e);
				numWatermarkIn = new SimpleCounter();
			}
		}

		if (numLatencyMarkersIn == null) {
			try {
				numLatencyMarkersIn = ((OperatorMetricGroup) streamOperator.getMetricGroup()).getIOMetricGroup().getNumLatencyMarkersIn();
			} catch (Exception e) {
				LOG.warn("An exception occurred during the metrics setup.", e);
				numLatencyMarkersIn = new SimpleCounter();
			}
		}
			...
			if (currentRecordDeserializer != null) {
				...
				if (result.isFullRecord()) {
					...
					if (recordOrMark.isWatermark()) {
						...
						if (schedulerCollector != null) {
							schedulerCollector.collect(recordOrMark.asWatermark());
						}
						continue;
						...
						synchronized (lock) {
							...
							if (schedulerCollector != null) {
								schedulerCollector.collect(record);
							}
						}
						return true;
					}
