...
public final class StreamElementSerializer<T> extends TypeSerializer<StreamElement> {
	...
	public void serialize(StreamElement value, DataOutputView target) throws IOException {
		if (value.isRecord()) {
			...
			target.writeLong(value.asWatermark().getIngestionTimestamp());
			target.writeBoolean(value.asWatermark().isEmitting());
		}
			...
			target.writeBoolean(value.asLatencyMarker().isBlocking());
		}
		else {
			throw new RuntimeException();
		}
	...
	public StreamElement deserialize(DataInputView source) throws IOException {
		...
		if (tag == TAG_REC_WITH_TIMESTAMP) {
			...
			return new Watermark(source.readLong(), source.readLong(), source.readBoolean());
		}
		else if (tag == TAG_STREAM_STATUS) {
			return new StreamStatus(source.readInt());
		}
		else if (tag == TAG_LATENCY_MARKER) {
			return new LatencyMarker(source.readLong(), new OperatorID(source.readLong(), source.readLong()), source.readInt(), source.readBoolean());
		}
		else {
			throw new IOException("Corrupt stream, found tag: " + tag);
		}
