...
public class WindowOperator<K, IN, ACC, OUT, W extends Window>
	...
	protected final HashMap<Window, Long> windowToCreationMap;

	private static final String LATE_ELEMENTS_DROPPED_METRIC_NAME = "numLateRecordsDropped";

	private static final String EMITTING_WATERMARK_DELAY_METRIC_NAME = "emittingWatermarkDelay";

	protected transient Counter numLateRecordsDropped;

	protected transient Histogram emittingWatermarkDelay;

	// This is only used to assert that things are working ok
	private final HashSet<Watermark> emittingWatermarks;

	protected Map<Window, Queue<LatencyMarker>> blockingLatencyMarkers = new HashMap<>();
	...
	/**
	 * Creates a new {@code WindowOperator} based on the given algorithm and user functions.
	 */
	public WindowOperator(
		...
		this.windowToCreationMap = new HashMap<>(4);
		this.emittingWatermarks = new HashSet<>();
	...
	public void open() throws Exception {
		...
		this.emittingWatermarkDelay = metrics.histogram(EMITTING_WATERMARK_DELAY_METRIC_NAME,
				new DescriptiveStatisticsHistogram(getMetricsHistorySize()));
...
//			}
	...
	public void processWatermark(Watermark mark) throws Exception {
		// Check if it is an emitting watermark.
		if (isEmittingWatermark(mark.getTimestamp())) {
			emittingWatermarks.add(mark);
			mark.setIsEmitting();
		}
		// super will be called later.
	...
	public void processElement(StreamRecord<IN> element) throws Exception {
		...
		if (windowAssigner instanceof MergingWindowAssigner) {
			...
			for (W window: elementWindows) {
				...
				if (!windowToCreationMap.containsKey(window)) {
					windowToCreationMap.put(window, System.currentTimeMillis());
				}
	...
	private void emitWindowContents(W window, ACC contents) throws Exception {
		...
		for (Watermark mark : emittingWatermarks) {
			if (mark.getTimestamp() == internalTimerService.currentWatermark()) {
				foundIt = true;
				emittingWatermarkDelay.update(
						mark.getIngestionTimestamp() - windowToCreationMap.get(window));
			}
		}
		if (!foundIt) {
			LOG.warn("Expected to find emitting watermark! but found {}", internalTimerService.currentWatermark());
		}

		// forward latency markers
		Queue<LatencyMarker> blockingLatencies = blockingLatencyMarkers.getOrDefault(window, new LinkedList<>());
		while (!blockingLatencies.isEmpty()) {
			this.output.emitLatencyMarker(blockingLatencies.poll());
		}
	}
	...
	@VisibleForTesting
	public List<W> getWindows(long watermark) {
		return new ArrayList<>(windowAssigner.assignWindows(null, watermark, windowAssignerContext));
	}

	@VisibleForTesting
	public boolean isEmittingWatermark(long newWatermark) {
		long currentWatermark = triggerContext.getCurrentWatermark();
		return currentWatermark > 0 &&
				newWatermark > currentWatermark &&
				!getWindows(currentWatermark).equals(getWindows(newWatermark));
	}
	...
	protected void reportOrForwardLatencyMarker(LatencyMarker marker) {
		...
		// Queue blocking latency markers until window is emitted
		if (marker.isBlocking()) {
			// Fetch the right windows
			long currentWatermark = triggerContext.getCurrentWatermark();
			if (currentWatermark <= 0) {
				// Ignore the early latency markers
				return;
			}
			List<W> windows = getWindows(currentWatermark);
			for (Window w : windows) {
				// update queue.
				Queue<LatencyMarker> markers = this.blockingLatencyMarkers.getOrDefault(w, new LinkedList<>());
				markers.offer(marker);
				this.blockingLatencyMarkers.put(w, markers);
			}
		} else {
	...
	public Histogram getEmittingWatermarkDelay() {
		return this.emittingWatermarkDelay;
	}
}
