...
public abstract class AbstractStreamOperator<OUT>
	...
	public void setup(StreamTask<?, ?> containingTask, StreamConfig config, Output<StreamRecord<OUT>> output) {
		...
		try {
			...
			this.output = new CountingOutput(output,
					...
					operatorMetricGroup.getIOMetricGroup().getNumWatermarksOut(),
					operatorMetricGroup.getIOMetricGroup().getNumLatencyMarkersOut()
					);
		...
		try {
			...
			int historySize = getMetricsHistorySize();
	...
	protected int getMetricsHistorySize() {
		Configuration taskManagerConfig = getContainingTask().getEnvironment().getTaskManagerInfo().getConfiguration();
	...
	public static class CountingOutput<OUT> implements Output<StreamRecord<OUT>> {
		...
		public CountingOutput(Output<StreamRecord<OUT>> output,
							  Counter recordsCounter, Counter watermarksCounter, Counter markersCounter) {
			this.output = output;
			this.numRecordsOut = recordsCounter;
			this.numWatermarksOut = watermarksCounter;
			this.numLatencyMarkersOut = markersCounter;
		}

		@Override
		public void emitWatermark(Watermark mark) {
			this.numWatermarksOut.inc();
			output.emitWatermark(mark);
		}

		@Override
		public void emitLatencyMarker(LatencyMarker latencyMarker) {
			numLatencyMarkersOut.inc();
