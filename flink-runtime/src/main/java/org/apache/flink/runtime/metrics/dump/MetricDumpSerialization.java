...
import static org.apache.flink.runtime.metrics.dump.QueryScopeInfo.*;
...
public class MetricDumpSerialization {
	...
	private static void serializeMetricInfo(DataOutput out, QueryScopeInfo info) throws IOException {
		...
		switch (info.getCategory()) {
			...
			case INFO_CATEGORY_OPERATOR:
				...
				break;
			case INFO_CATEGORY_TS:
				QueryScopeInfo.TaskSchedulerQueryScopeInfo schedulerInfo = (QueryScopeInfo.TaskSchedulerQueryScopeInfo) info;
				out.writeUTF(schedulerInfo.taskManagerID);
				out.writeUTF(schedulerInfo.schedulerName);
				break;
			default:
	...
	private static MetricDump.HistogramDump deserializeHistogram(DataInputView dis) throws IOException {
		...
		return new MetricDump.HistogramDump(info, name, count, min, max, mean, median, stddev, p75, p90, p95, p98, p99, p999);
	}
	...
	private static QueryScopeInfo deserializeMetricInfo(DataInput dis) throws IOException {
		...
		switch (cat) {
			...
			case INFO_CATEGORY_TS:
				tmID = dis.readUTF();
				String schedulerName = dis.readUTF();
				return new QueryScopeInfo.TaskSchedulerQueryScopeInfo(tmID, schedulerName, scope);
			default:
