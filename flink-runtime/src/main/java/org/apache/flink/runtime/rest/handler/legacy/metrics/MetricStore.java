...
import static org.apache.flink.runtime.metrics.dump.QueryScopeInfo.INFO_CATEGORY_TS;
		...
		try {
			...
			switch (info.getCategory()) {
				...
				case INFO_CATEGORY_TS:
					QueryScopeInfo.TaskSchedulerQueryScopeInfo tsInfo = (QueryScopeInfo.TaskSchedulerQueryScopeInfo) info;
					tm = taskManagers.computeIfAbsent(tsInfo.taskManagerID, k -> new TaskManagerMetricStore());
					addMetric(tm.metrics, name, metric);
					break;
	...
	private void addMetric(Map<String, String> target, String name, MetricDump metric) {
		switch (metric.getCategory()) {
			...
			case METRIC_CATEGORY_HISTOGRAM:
				...
				target.put(name + "_count", String.valueOf(histogram.count));
