...
import org.apache.flink.runtime.taskexecutor.scheduler.RuntimeTaskScheduler;
import org.apache.flink.runtime.taskexecutor.scheduler.TaskSchedulerUtils;
...
public class TaskExecutor extends RpcEndpoint implements TaskExecutorGateway {
	...
	private final RuntimeTaskScheduler runtimeTaskScheduler;
	...
	public TaskExecutor(
			...
			RuntimeTaskScheduler runtimeTaskScheduler) {
		...
		this.runtimeTaskScheduler = checkNotNull(runtimeTaskScheduler);
	...
	public CompletableFuture<Acknowledge> submitTask(
		...
		try {
            ...
            /* Setup taskScheduler. */
            if (!runtimeTaskScheduler.isSetup()) {
                TaskSchedulerMetricGroup taskSchedulerMetricGroup =
                        taskManagerMetricGroup.addSchedulerMetrics(jobInformation.getTaskSchedulerClassName());
                try {
                    ClassLoader classLoader = TaskSchedulerUtils.createClassLoader(jobInformation.getJobId(),
                            tdd.getExecutionAttemptId(), jobInformation.getRequiredJarFileBlobKeys(),
                            jobInformation.getRequiredClasspathURLs(), libraryCache);
                    runtimeTaskScheduler.setup(taskSchedulerMetricGroup, classLoader,
                            jobInformation.getTaskSchedulerClassName(), jobInformation.getTaskSchedulerPolicy());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
			...
			if (taskAdded) {
				...
				runtimeTaskScheduler.addTask(task);
	...
	public CompletableFuture<Acknowledge> cancelTask(ExecutionAttemptID executionAttemptID, Time timeout) {
		...
		if (task != null) {
			try {
                ...
                runtimeTaskScheduler.removeTask(task);
				return CompletableFuture.completedFuture(Acknowledge.get());
			} catch (Throwable t) {
				return FutureUtils.completedExceptionally(
