 ...
 * not contain tasks any more as well as initializing the TaskScheduler group metrics.
 */
@Internal
public class TaskManagerMetricGroup extends ComponentMetricGroup<TaskManagerMetricGroup> {
	...
	/** Map for scheduler name to scheduler metrics. */
	private final Map<String, TaskSchedulerMetricGroup> schedulers = new HashMap<>();
	...
	//  scheduler
	// ------------------------------------------------------------------------
	public TaskSchedulerMetricGroup addSchedulerMetrics(final String schedulerName) {
		synchronized (this) {
			if (!isClosed()) {
				TaskSchedulerMetricGroup prior = schedulers.get(schedulerName);
				if (prior != null) {
					LOG.warn("Attempting to add multiple schedulers metric groups to the same task.");
					return prior;
				} else {
					TaskSchedulerMetricGroup scheduler = new TaskSchedulerMetricGroup(this.registry, this, schedulerName);
	...
	protected Iterable<? extends ComponentMetricGroup> subComponents() {
		return Stream.concat(jobs.values().stream(), schedulers.values().stream()).collect(Collectors.toList()); }
