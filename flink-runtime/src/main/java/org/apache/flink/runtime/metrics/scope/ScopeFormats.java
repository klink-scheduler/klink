...
public final class ScopeFormats {
	...
	private final TaskSchedulerScopeFormat schedulerFormat;
	...
	private ScopeFormats(
		...
		this.schedulerFormat = new TaskSchedulerScopeFormat(schedulerFormat, this.taskFormat);
	}
	...
	public TaskSchedulerScopeFormat getSchedulerFormat() { return this.schedulerFormat; }
	...
	public static ScopeFormats fromConfig(Configuration config) {
		...
		/*
		 * This should ideally be moved to configuration module but since we are trying
		 * to minimize the number of changed modules, we hard-code the naming of this
		 * MetricOptions.
		 */
		String schedulerFormat = "metrics.scope.scheduler";
		return new ScopeFormats(jmFormat, jmJobFormat, tmFormat, tmJobFormat,
				taskFormat, operatorFormat, schedulerFormat);
	}
}
